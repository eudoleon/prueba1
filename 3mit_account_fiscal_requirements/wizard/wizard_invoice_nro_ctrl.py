...
class WizardInvoiceNroCtrl(models.TransientModel):
    ...
    nro_ctrl = fields.Char(string='Número de Control', size=32,
                           help="Número utilizado para gestionar facturas preimpresas, por ley Necesito poner este "
                                "número aquí para declarar Informes de impuestos correctamente.", store=True)
    ...
    def action_invoice_create(self, wizard_brw, inv_brw):
        ...
        # @param wizard_brw: nothing for now @param inv_brw: damaged paper
        ...
        if inv_brw.nro_ctrl:
            ...
            raise exceptions.except_orm(_('Error de Validación!'), _("Puede ejecutar este proceso solo si la factura "
                                                                     "tiene Numero de Control, verifique la factura e "
                                                                     "intente nuevamente."))
    ...
    def create_invoice(self):
        ...
        for wizard in wizard_brw:
            if not wizard.sure:
                raise exceptions.except_orm(_("Validation error!"), _("Please confirm that you know what you're doing "
                                                                      "by checking the option bellow!"))
            if (wizard.invoice_id and wizard.invoice_id.company_id.jour_id and
                ...
                raise exceptions.except_orm(_('Error de Validación!'), _("Debe ir al formulario de empresa y configurar"
                                                                         " un diario y una cuenta para facturas dañadas"))
